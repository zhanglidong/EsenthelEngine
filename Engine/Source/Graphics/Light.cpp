...
static Bool SetLum()
{
   ...
   if(set)
   {
      D.depth2DOff(); D.clearCol((Renderer._lum_1s!=Renderer._lum || (Renderer._ao && !D._amb_all)) ? Vec4Zero : Vec4(D.ambientColorL(), 0));
      D.depth2DOn ();
   }
   ...
   switch(CurrentLight.type)
   {
      case LIGHT_DIR:
      {
         ...
         }else // multi-sample
         {
            ...
            if(clear)
            {
               D.depth2DOff(); D.stencil(STENCIL_NONE); D.clearCol((Renderer._ao && !D._amb_all) ? Vec4Zero : Vec4(D.ambientColorL(), 0));
               D.depth2DOn ();
            }
