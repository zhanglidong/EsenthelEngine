...
Shader* MainShaderClass::getSunRays    (Bool high, Bool jitter, Bool gamma) {return get(S8+"SunRays"    +(high?'H':'\0')+(jitter?'J':'\0')+(gamma?'G':'\0'));}
...
void MainShaderClass::getTechniques()
{
   ...
   ConstCast(Renderer.ambient_color_l)=GetShaderParam("AmbColor"       ); Renderer.ambient_color_l->set(D.ambientColorL   ());
 ...
 //h_DrawTexNrm =get("DrawTexNrm"); used by Editor
   ...
   REPD(j, 2)
   REPD(g, 2)
      h_SunRays[h][j][g]=getSunRays(h, j, g);
