...
Bool RendererClass::motionBlur(ImageRT &src, ImageRT &dest, Bool alpha, Bool combine)
{
   ...
   // Dilate
   {
    ...
    //Sh.imgSize(*dilated); we can just use 'RTSize' instead of 'ImgSize' since there's no scale
      ...
      Int dilate_pixels=DivRound((UInt)rt_desc.size.y, (UInt)4*2); // dilate up to 25% of the screen vertically (this is the max amount of blur length, *2 because we blur both ways) #MaxMotionBlurLength
      if( dilate_pixels>0)
      {
