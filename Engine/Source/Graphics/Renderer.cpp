   ...
   if(hasEdgeSoften())
   {
      ...
      switch(D.edgeSoften())
      {
         case EDGE_SOFTEN_FXAA:
         {
            Bool gamma=false;
         #if LINEAR_GAMMA
            Bool swap=(_col->canSwapSRGB() && dest->canSwapSRGB()); if(swap){dest->swapRTV(); _col->swapSRV();} // if we have a non-sRGB access, then just use it instead of doing the more expensive shader, later we have to restore it
            else gamma=true;
         #endif
         ...
         case EDGE_SOFTEN_SMAA:
         {
            ...
            Bool gamma=false;
         #if LINEAR_GAMMA
            if(_col->canSwapSRGB())_col->swapSRV(); // if we have a non-sRGB access, then just use it instead of doing the more expensive shader, later we have to restore it
            else                   gamma=true;
         #endif
            D.stencil(STENCIL_EDGE_SOFT_SET, STENCIL_REF_EDGE_SOFT); // have to use '_ds_1s' in write mode to be able to use stencil
         ...
         #if LINEAR_GAMMA
            if(_col->canSwapSRGB())_col->swapSRV(); // restore
         #endif
            ...
            Bool swap=(dest->canSwapSRGB() && _col->canSwapSRGB()); if(swap){dest->swapRTV(); _col->swapSRV();}
         #endif
