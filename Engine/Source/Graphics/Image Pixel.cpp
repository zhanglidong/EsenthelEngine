...
static Color DecompressPixel(C Image &image, Int x, Int y)
{
   switch(image.hwType())
   {
      ...
      case IMAGE_BC4    :                          return DecompressPixelBC4   (image.data() + (x>>2)*16 + (y>>2)*image.pitch(), x&3, y&3);
      case IMAGE_BC5    :                          return DecompressPixelBC5   (image.data() + (x>>2)*16 + (y>>2)*image.pitch(), x&3, y&3);
...
static Color DecompressPixel(C Image &image, Int x, Int y, Int z)
{
   switch(image.hwType())
   {
      ...
      case IMAGE_BC4    :                          return DecompressPixelBC4   (image.data() + (x>>2)*16 + (y>>2)*image.pitch() + z*image.pitch2(), x&3, y&3);
      case IMAGE_BC5    :                          return DecompressPixelBC5   (image.data() + (x>>2)*16 + (y>>2)*image.pitch() + z*image.pitch2(), x&3, y&3);
...
static void _SetColorS(Byte *data, IMAGE_TYPE type, C Vec4 &color)
{
   switch(type)
   {
    ...
    /*case IMAGE_R8_SIGN      : (*(Byte *)data)=     SFltToSByte(SRGBToLinear(color.x))                                                                                               ; break; TODO:
      ...
      case IMAGE_R8G8B8A8_SIGN: ( (VecB4*)data)->set(SFltToSByte(SRGBToLinear(color.x)), SFltToSByte(SRGBToLinear(color.y)), SFltToSByte(SRGBToLinear(color.z)), SFltToSByte(color.w)); break;*/
...
static inline Vec4 GetColorS(CPtr data, C Image &image, Bool _2d, Int x, Int y, Int z=0)
{
   switch(image.hwType())
   {
    ...
    /*case IMAGE_R8_SIGN      : {Byte  &c=*(Byte *)data; return Vec4(LinearToSRGB(SByteToSFlt(c  )),                              0,                              0,                1);} TODO:
      ...
      case IMAGE_R8G8B8A8_SIGN: {VecB4 &c=*(VecB4*)data; return Vec4(LinearToSRGB(SByteToSFlt(c.x)), LinearToSRGB(SByteToSFlt(c.y)), LinearToSRGB(SByteToSFlt(c.z)), SByteToSFlt(c.w));}*/
