...
static void SetColorF(Byte *data, IMAGE_TYPE type, C Vec4 &color)
{
   switch(type)
   {
      ...
      case IMAGE_R10G10B10A2: {(*(UInt*)data)=FltToU10(color.x)|(FltToU10(color.y)<<10)|(FltToU10(color.z)<<20)|(Mid(RoundPos(color.w*3), 0, 3)<<30);} break;
   }
}
...
Vec4 ImageColorF(CPtr data, IMAGE_TYPE hw_type)
{
   switch(hw_type)
   {
    ...
    //case IMAGE_D32I: if(GL)return Vec4(Vec((*(U32*)data)/Dbl(0xFFFFFFFFu)*2-1), 1); // !! else fall through no break on purpose !!
      ...
      case IMAGE_R10G10B10A2: {UInt u=*(UInt*)data; return Vec4(U10ToFlt(u&0x3FF), U10ToFlt((u>>10)&0x3FF), U10ToFlt((u>>20)&0x3FF), (u>>30)/3.0f);}
   }
   return 0;
}
