...
/******************************************************************************/
Quad  ::Quad  (C QuadD  &quad) {p[0]=quad.p[0]; p[1]=quad.p[1]; p[2]=quad.p[2]; p[3]=quad.p[3];             }
QuadN ::QuadN (C Quad   &quad) {p[0]=quad.p[0]; p[1]=quad.p[1]; p[2]=quad.p[2]; p[3]=quad.p[3]; setNormal();}
QuadN ::QuadN (C QuadND &quad) {p[0]=quad.p[0]; p[1]=quad.p[1]; p[2]=quad.p[2]; p[3]=quad.p[3]; n=quad.n   ;}
QuadD ::QuadD (C Quad   &quad) {p[0]=quad.p[0]; p[1]=quad.p[1]; p[2]=quad.p[2]; p[3]=quad.p[3];             }
QuadND::QuadND(C QuadD  &quad) {p[0]=quad.p[0]; p[1]=quad.p[1]; p[2]=quad.p[2]; p[3]=quad.p[3]; setNormal();}
QuadND::QuadND(C Quad   &quad) {p[0]=quad.p[0]; p[1]=quad.p[1]; p[2]=quad.p[2]; p[3]=quad.p[3]; setNormal();} // recalculate for high precision instead of using 'quad.n'
...
Quad  & Quad  ::operator-=(C Vec   &v) {p[0]-=v; p[1]-=v; p[2]-=v; p[3]-=v; return T;}
QuadN & QuadN ::operator+=(C Vec   &v) {p[0]+=v; p[1]+=v; p[2]+=v; p[3]+=v; return T;}
QuadN & QuadN ::operator-=(C Vec   &v) {p[0]-=v; p[1]-=v; p[2]-=v; p[3]-=v; return T;}
QuadD & QuadD ::operator+=(C VecD  &v) {p[0]+=v; p[1]+=v; p[2]+=v; p[3]+=v; return T;}
QuadD & QuadD ::operator-=(C VecD  &v) {p[0]-=v; p[1]-=v; p[2]-=v; p[3]-=v; return T;}
QuadND& QuadND::operator+=(C VecD  &v) {p[0]+=v; p[1]+=v; p[2]+=v; p[3]+=v; return T;}
QuadND& QuadND::operator-=(C VecD  &v) {p[0]-=v; p[1]-=v; p[2]-=v; p[3]-=v; return T;}
...
Quad  & Quad  ::operator/=(  Flt    r) {p[0]/=r; p[1]/=r; p[2]/=r; p[3]/=r; return T;}
QuadN & QuadN ::operator*=(  Flt    r) {p[0]*=r; p[1]*=r; p[2]*=r; p[3]*=r; return T;}
QuadN & QuadN ::operator/=(  Flt    r) {p[0]/=r; p[1]/=r; p[2]/=r; p[3]/=r; return T;}
QuadD & QuadD ::operator*=(  Dbl    r) {p[0]*=r; p[1]*=r; p[2]*=r; p[3]*=r; return T;}
QuadD & QuadD ::operator/=(  Dbl    r) {p[0]/=r; p[1]/=r; p[2]/=r; p[3]/=r; return T;}
QuadND& QuadND::operator*=(  Dbl    r) {p[0]*=r; p[1]*=r; p[2]*=r; p[3]*=r; return T;}
QuadND& QuadND::operator/=(  Dbl    r) {p[0]/=r; p[1]/=r; p[2]/=r; p[3]/=r; return T;}
/******************************************************************************/
...
{
   return Abs(DistPointPlane(quad.p[0], T))<=EPS
       && Abs(DistPointPlane(quad.p[1], T))<=EPS
       && Abs(DistPointPlane(quad.p[2], T))<=EPS
       && Abs(DistPointPlane(quad.p[3], T))<=EPS;
}
/******************************************************************************/
...
Bool Cuts(C Vec &point, C Quad &quad, Bool test_quads_as_2_tris)
{
   ...
   }else
   {
      ...
      Vec p=point-quad.p[1]; if(DistPointPlane(p, Cross(quad_nu, quad.p[0]-quad.p[1]))>0)return false;
                             if(DistPointPlane(p, Cross(quad_nu, quad.p[1]-quad.p[2]))>0)return false;
          p=point-quad.p[3]; if(DistPointPlane(p, Cross(quad_nu, quad.p[2]-quad.p[3]))>0)return false;
                             if(DistPointPlane(p, Cross(quad_nu, quad.p[3]-quad.p[0]))>0)return false;
   }
   return true;
}
