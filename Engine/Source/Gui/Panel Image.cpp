...
namespace EE{
#define PANEL_IMAGE_LINEAR 0 // if perform operations in Linear Gamma (if false then sRGB Gamma), false generated prettier results
static inline Vec4 ConvertSRGB  (C Color &srgb) {return PANEL_IMAGE_LINEAR ? SRGBToLinear(srgb) : (Vec4)srgb;}
static inline Vec  ConvertLinear(C Vec   &lin ) {return PANEL_IMAGE_LINEAR ? lin :      LinearToSRGB(lin    );}
static inline Vec4 ConvertLinear(C Vec4  &lin ) {return PANEL_IMAGE_LINEAR ? lin : Vec4(LinearToSRGB(lin.xyz), lin.w);}
/******************************************************************************
      ...
      REPD(x, map.w())
      {
        ...
        SAT(dist_depth_sect*=2); // 'dist_depth_sect' is now 0 .. 1.0
          ...
          C PanelImageParams::Section &section  =params.sections[section_i];
            ...
            if(sp.overlay)
            {
               Vec4 ovr=ConvertLinear(sp.overlay->colorFCubicFast(x*sp.overlay_uv_scale+sp.overlay_uv_offset.x,
   ...
   void setColorY(Int y)
   {
      ...
      REPD(x, image_size.x)
      {
       ...
       C Vec4 &dist4 =dist_map.pixF4(x, y);
         ...
         if(   dist_global>=0)
         {
          ...
          C PanelImageParams::Section &section=params.sections[section_i];
            ...
            if(sp.overlay)
            {
               ...
               Vec4 ovr=ConvertLinear(sp.overlay->colorFCubicFast(tex.x, tex.y, false));
            ...
            if(y<y1)
            {
               if(x<top_x1){if( top_left_image)col*=ConvertLinear( top_left_image->colorFCubicFast(x*image_frac_mul_add[0][0].x+image_frac_mul_add[0][0].y, image_frac_y[0][0], true, true));}else
               if(x<top_x2){if(      top_image)col*=ConvertLinear(      top_image->colorFCubicFast(x*image_frac_mul_add[1][0].x+image_frac_mul_add[1][0].y, image_frac_y[1][0], true, true));}else
                           {if(top_right_image)col*=ConvertLinear(top_right_image->colorFCubicFast(x*image_frac_mul_add[2][0].x+image_frac_mul_add[2][0].y, image_frac_y[2][0], true, true));}
            }else
            if(y<y2)
            {
               if(x<x1){if(  left_image)col*=ConvertLinear(  left_image->colorFCubicFast(x*image_frac_mul_add[0][1].x+image_frac_mul_add[0][1].y, image_frac_y[0][1], true, true));}else
               if(x<x2){if(center_image)col*=ConvertLinear(center_image->colorFCubicFast(x*image_frac_mul_add[1][1].x+image_frac_mul_add[1][1].y, image_frac_y[1][1], true, true));}else
                       {if( right_image)col*=ConvertLinear( right_image->colorFCubicFast(x*image_frac_mul_add[2][1].x+image_frac_mul_add[2][1].y, image_frac_y[2][1], true, true));}
            }else
            {
               if(x<bottom_x1){if( bottom_left_image)col*=ConvertLinear( bottom_left_image->colorFCubicFast(x*image_frac_mul_add[0][2].x+image_frac_mul_add[0][2].y, image_frac_y[0][2], true, true));}else
               if(x<bottom_x2){if(      bottom_image)col*=ConvertLinear(      bottom_image->colorFCubicFast(x*image_frac_mul_add[1][2].x+image_frac_mul_add[1][2].y, image_frac_y[1][2], true, true));}else
                              {if(bottom_right_image)col*=ConvertLinear(bottom_right_image->colorFCubicFast(x*image_frac_mul_add[2][2].x+image_frac_mul_add[2][2].y, image_frac_y[2][2], true, true));}
            }
            ...
            if(sp.reflection)
            {
               ...
               Vec tex=ConvertLinear(sp.reflection->colorFCubicFast(uv.x, uv.y).xyz);
         ...
         if(!PANEL_IMAGE_LINEAR)col.xyz=SRGBToLinear(col.xyz); // convert from sRGB to Linear
