...
void XBOXLive::getUserProfile(ULong user_id)
{
   ...
   if(user_id && XboxCtx)
   {
      ...
      XboxCtx->profile_service().get_user_profile(temp).then([this](xbox::services::xbox_live_result<xbox::services::xbox_live_result<xbox::services::social::xbox_user_profile>> results)
      {
         if(!results.err())
         {
...
void XBOXLive::logInOk()
{
 ...
 //Str age=XboxUser->age_group().c_str(); // can return: "Adult", ..
   ...
   setStatus(LOGGED_IN); // call after 'XboxCtx' and setting members
