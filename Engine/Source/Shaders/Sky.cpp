...
):TARGET
{
...
#if   TEXTURES==2
   col.rgb=Lerp(TexCube(Cub, inTex).rgb, TexCube(Cub1, inTex).rgb, SkyBoxBlend);
#elif TEXTURES==1
   col.rgb=TexCube(Cub, inTex).rgb;
#else
   #if PER_VERTEX
      col=inCol;
   #else
   ...
   #if STARS
      col.rgb=Lerp(TexCube(Cub, inTexStar).rgb, col.rgb, col.a);
   #endif
#endif
