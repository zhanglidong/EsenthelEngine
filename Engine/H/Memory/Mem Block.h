...
T1(const_mem_addr TYPE) struct Memb : _Memb // Block Based Container
{
 ...
 C TYPE& last      (     )C; // get    last  element
   ...
   Memb& sort(CPtr user, Int compare(C TYPE &a, C TYPE &b, CPtr user)        ); // sort elements with custom comparing function and 'user' parameter, this method may change the memory address of all elements
