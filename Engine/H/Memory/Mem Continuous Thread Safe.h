...
T1(const_mem_addr TYPE) struct MemcThreadSafe : _MemcThreadSafe // Thread-Safe Continuous Memory Based Container
{
 ...
 C TYPE& lockedLast (     )C; // get    last  element, container must be locked first using the 'lock' method before using this method
   ...
   T1(VALUE) MemcThreadSafe& lockadAdd    (C VALUE &value                       )  {                                 lockedNew()=value ; return T;                                                                   } // add      'value' to container                                                                                       , this method may change the memory address of all elements, container must be locked first using the 'lock' method before using this method
   T1(VALUE) Bool                  include(C VALUE &value                       )  {SyncLocker locker(_lock); if(!has(value)){lockedAdd(value); return true;} return false;                                          } // include  'value' if it's not already present in container, returns true if value wasn't present and has been added  , this method may change the memory address of all elements
   T1(VALUE) Bool                  exclude(C VALUE &value, Bool keep_order=false)  {SyncLocker locker(_lock); Int i=find(value); if(i>=0){lockedRemove(i, keep_order); return true ;}                   return false;} // exclude  'value' if present  in container                , returns true if value was    present and has been removed, this method may change the memory address of all elements
   T1(VALUE) Bool                  toggle (C VALUE &value, Bool keep_order=false)  {SyncLocker locker(_lock); Int i=find(value); if(i>=0){lockedRemove(i, keep_order); return false;} lockedAdd(value); return true ;} // toggle   'value'    presence in container                , returns true if value is now present in container        , this method may change the memory address of all elements
