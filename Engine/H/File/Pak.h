...
// Update Pak
Bool PakUpdate       (Pak &src_pak, C CMemPtr<PakFileData> &update_files, C Str &pak_name, UInt flag=0, Cipher *dest_cipher=null, COMPRESS_TYPE compress=COMPRESS_NONE, Int compression_level=9, Str *error_message=null, PakProgress *progress=null); // update 'src_pak' by adding 'update_files' and saving the new Pak into 'pak_name', 'compression_level'=0..CompressionLevels(compress) (0=fastest/worst, ..=slowest/best), 'compress'=compression algorithm for the 'update_files' (files from 'src_pak'  will preserve their original compression), this completely recreates the Pak fully rewriting its data            , for example updating 1GB Pak with 1MB file will perform 1GB+ operations, this will result in a lot of Disk operations            , however created Pak will     be fully optimized, and as small as possible
Bool PakUpdateInPlace(              C CMemPtr<PakFileData> &update_files, C Str &pak_name, UInt flag=0, Cipher *     cipher=null, COMPRESS_TYPE compress=COMPRESS_NONE, Int compression_level=9, Str *error_message=null, PakProgress *progress=null); // update 'pak_name' Pak by adding 'update_files' in-place                         , 'compression_level'=0..CompressionLevels(compress) (0=fastest/worst, ..=slowest/best), 'compress'=compression algorithm for the 'update_files' (files from 'pak_name' will preserve their original compression), this operates on existing Pak, adjusting it without creating new files, for example updating 1GB Pak with 1MB file will perform 1MB+ operations, it   will perform as few     Disk operations as possible, however created Pak will not be fully optimized, and may contain some useless data
