...
struct VtxFormatKey
{
   MeshFlag flag    ; // MESH_BASE_FLAG
   UInt     compress; // VTX_COMPRESS_FLAG

   VtxFormatKey() {}
   VtxFormatKey(MeshFlag flag, UInt compress) {T.flag=flag; T.compress=compress;}
};
#if GL
...
struct VtxFormat // Vertex Format
{
   ...
   #endif
   Bool create(MeshFlag flag, UInt compress); // 'flag'=MESH_BASE_FLAG, 'compress'=VTX_COMPRESS_FLAG
#if GL
          void bind(C VtxBuf &vb);
#else
...
struct VtxBuf // Vertex Buffer
{
 ...
 C Byte*  lockRead  ()C;
   ...
   Bool create   (Int vtx_num , MeshFlag flag, UInt compress, Bool dynamic=false                ); // 'flag'=MESH_BASE_FLAG, 'compress'=VTX_COMPRESS_FLAG, false on fail
