...
void RenderIcon(void (&render)(), C ViewSettings *view, ImageRT &image, C VecI2 &image_size, Int super_sample=1); // create icon by performing rendering into 'image', 'render'=rendering function, 'view'=view settings (optional, if null then current 'D.view' settings will be used), 'image'=image render target, 'image_size'=image size in pixels, 'super_sample'=super sampling to be used for rendering (1=disabled, 2=2x, 4=4x, ..), this function ignores 'D.density', 'D.maxLights', 'D.eyeAdaptation', 'D.lodFactor', 'D.motionMode', 'D.tAA'. This function can be called at any time (in State Update, Draw, Init, Shut) however only on the main thread.
