...
struct DisplayClass : DisplayState, DisplayDraw // Display Control
{
 ...
 C Str8&        deviceName     ()C {return _device_name   ;} // get GPU device name
   ...
   DisplayClass& texMipBias       (Flt              bias        );   Flt              texMipBias        ()C {return _tex_mip_bias     ;} // set/get Texture MipMap Bias                    (-Inf..Inf                          , default=                0                             ), values bigger than zero indicate lower texture quality (smaller mip maps will be used), but faster rendering, the change is NOT instant, avoid calling real-time
   ...
   DisplayClass& bloomScale   (Flt  scale   );   Flt  bloomScale   ()C {return _bloom_scale   ;} // set/get Bloom Scale           (   0..Inf , default=0.5                     ), the change is instant, you can call it real-time
   ...
   DisplayClass& ambientPowerS  (  Flt        srgb_power);   Flt          ambientPowerS  ()C;                             // set/get Ambient Power sRGB   Gamma (0..1                 , default=         0.4), this is equivalent to using 'ambientColorS' with RGB components set to the same value, the change is instant, you can call it real-time
   ...
   DisplayClass& ambientColorS  (C Vec       &srgb_color);   Vec          ambientColorS  ()C;                             // set/get Ambient Color sRGB   Gamma (0..1                 , default=         0.4), the change is instant, you can call it real-time
   DisplayClass& ambientContrast(  Flt        contrast  );   Flt          ambientContrast()C {return _amb_contrast     ;} // set/get Ambient Contrast           (0..Inf               , default=       4.375), the change is instant, you can call it real-time
