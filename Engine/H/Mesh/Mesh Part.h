...
struct MeshPart // Mesh Base + Mesh Render
{
 ...
 C MaterialPtr& multiMaterial(Int i)C; // get i-th multi material
   ...
   // draw
      ...
      void draw(C MatrixM          &matrix, C Vec &vel=VecZero, C Vec &ang_vel=VecZero)C; // add mesh to draw list using 'matrix'    matrix, 'vel' velocity and 'ang_vel' angular velocity, this should be called only in RM_PREPARE, when used it will automatically draw meshes in following modes when needed: RM_EARLY_Z RM_SOLID RM_SOLID_M RM_AMBIENT RM_BLEND
