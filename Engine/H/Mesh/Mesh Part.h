...
struct MeshPart // Mesh Base + Mesh Render
{
 ...
 C MaterialPtr& multiMaterial(Int i)C; // get i-th multi material
   ...
   // draw
      ...
      void draw(C MatrixM          &matrix, C Vec &pos_delta=VecZero, C Vec &ang_delta=VecZero)C; // add mesh to draw list using 'matrix'    matrix, 'pos_delta' position delta (from previous to current frame) and 'ang_delta' angle delta (from previous to current frame), this should be called only in RM_PREPARE, when used it will automatically draw meshes in following modes when needed: RM_EARLY_Z RM_SOLID RM_SOLID_M RM_AMBIENT RM_BLEND
