...
struct MeshRender // Mesh Renderable (Hardware: contains Vertexes + Triangles)
{
   ...
   Bool create   (C MeshRender  *src[], Int elms, MeshFlag flag_and=MESH_ALL, Bool optimize=false, Bool compress=true); // create from 'src' array, 'flag_and'=data to copy
#endif
   ...
   Int  vtxOfs  (MeshFlag elm    )C;                                        // get offset of a specified vertex component in the vertex data, -1 if not found
   Bool indBit16(                )C {return _ib.bit16()                  ;} // if  indices are 16-bit (false for 32-bit)
   MeshFlag flag(                )C {return _flag                        ;} // get available data
 ...
 C Byte* vtxLockedElm (MeshFlag  elm                 )C;                               //    get vertex data if it's already locked offsetted by specified vertex component in the vertex data according to 'vtxOfs' method, null on fail (if the vertex buffer is not locked or if the component was not found)
