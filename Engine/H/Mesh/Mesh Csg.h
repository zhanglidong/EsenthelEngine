...
void  ClipMesh     (C MeshBase &src, C Matrix *matrix, MeshBase &dest         ,                      C Plane *clip_plane, Int clip_planes,                          MeshFlag flag_and=MESH_ALL, Flt weld_pos_eps=EPS); // generate 'src' mesh transformed by 'matrix' and then clipped by 'clip_planes' (if the 'matrix' is null then the mesh is not transformed), result is stored in 'dest'                             , 'weld_pos_eps'=epsilon used for final vertex position welding, 'flag_and'=elements to include in 'dest'   creation (MESH_FLAG)
void  ClipMesh     (C Mesh     &src, C Matrix *matrix, Mesh     &dest         ,                      C Plane *clip_plane, Int clip_planes,                          MeshFlag flag_and=MESH_ALL, Flt weld_pos_eps=EPS); // generate 'src' mesh transformed by 'matrix' and then clipped by 'clip_planes' (if the 'matrix' is null then the mesh is not transformed), result is stored in 'dest'                             , 'weld_pos_eps'=epsilon used for final vertex position welding, 'flag_and'=elements to include in 'dest'   creation (MESH_FLAG)
void SplitMesh     (C Mesh     &src, C Matrix *matrix, Mesh     &dest_positive, Mesh &dest_negative, C Plane &clip_plane,                                           MeshFlag flag_and=MESH_ALL, Flt weld_pos_eps=EPS); // generate 'src' mesh transformed by 'matrix' and then split   by 'clip_plane'  (if the 'matrix' is null then the mesh is not transformed), result is stored in 'dest_positive' and 'dest_negative', 'weld_pos_eps'=epsilon used for final vertex position welding, 'flag_and'=elements to include in 'dest_*' creation (MESH_FLAG)
void SplitMeshSolid(C Mesh     &src, C Matrix *matrix, Mesh     &dest_positive, Mesh &dest_negative, C Plane &clip_plane, C MaterialPtr &material, Flt tex_scale=1, MeshFlag flag_and=MESH_ALL, Flt weld_pos_eps=EPS); // generate 'src' mesh transformed by 'matrix' and then split   by 'clip_plane'  (if the 'matrix' is null then the mesh is not transformed), result is stored in 'dest_positive' and 'dest_negative', 'weld_pos_eps'=epsilon used for final vertex position welding, 'flag_and'=elements to include in 'dest_*' creation (MESH_FLAG), 'material'=material used to fill the "solid inside" created because of the splitting, 'tex_scale'=texture coordinates scale of the "solid inside" part
/******************************************************************************/
