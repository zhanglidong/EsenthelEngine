...
struct Mesh : MeshLod // Mesh (array of Mesh Lod's)
{
 ...
 C MeshLod& getDrawLod (  Flt      dist2 )C; // get          Level of Detail which should be used for drawing with current camera and given object 'dist2' squared distance to camera which can be calculated using 'GetLodDist2' function
   ...
   // draw
      ...
      void draw(C Matrix           &matrix, C Vec &pos_delta, C Vec &ang_delta=VecZero)C; // add mesh to draw list using 'matrix'    matrix, 'pos_delta' position delta (from previous to current frame) and 'ang_delta' angle delta (from previous to current frame), this should be called only in RM_PREPARE, when used it will automatically draw meshes in following modes when needed: RM_EARLY_Z RM_SOLID RM_SOLID_M RM_AMBIENT RM_BLEND
      void draw(C MatrixM          &matrix, C Vec &pos_delta, C Vec &ang_delta=VecZero)C; // add mesh to draw list using 'matrix'    matrix, 'pos_delta' position delta (from previous to current frame) and 'ang_delta' angle delta (from previous to current frame), this should be called only in RM_PREPARE, when used it will automatically draw meshes in following modes when needed: RM_EARLY_Z RM_SOLID RM_SOLID_M RM_AMBIENT RM_BLEND
      ...
      void drawBlend(C MatrixM          &matrix, C Vec &pos_delta=VecZero, C Vec &ang_delta=VecZero, C Vec4 *color=null)C; // draw with 'matrix'    matrix  , 'pos_delta' position delta (from previous to current frame) and 'ang_delta' angle delta (from previous to current frame), 'color'=pointer to optional Material color multiplication
