...
struct Mesh : MeshLod // Mesh (array of Mesh LODs)
{
 ...
 C MeshLod& lod (Int i)C {return i ? _lods[i-1] : SCAST(C MeshLod, T);} // return  i-th  Level of Detail
   ...
   Mesh& create(C Mesh      &src        , MeshFlag flag_and=MESH_ALL); // create from 'src'     , 'flag_and'=MESH_FLAG
   Mesh& create(C MeshGroup &src        , MeshFlag flag_and=MESH_ALL); // create from 'src'     , 'flag_and'=MESH_FLAG
   Mesh& create(C Mesh *mesh, Int meshes, MeshFlag flag_and=MESH_ALL); // create from mesh array, 'flag_and'=MESH_FLAG
