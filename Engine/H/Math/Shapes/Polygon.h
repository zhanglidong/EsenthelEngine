...
void Triangulate(        C CMemPtr<VtxFull>   &poly , MeshBase &mesh, MeshFlag flag_and,                       Bool convex=false                         ); // triangulate         'poly' to 'mesh', 'mesh' will be deleted at start, 'flag_and'=elements to include in 'mesh' creation                                                               , 'convex'=if 'polys' are known to be convex (this will speed up the process)
...
void Triangulate(C CMemPtr< Memc  <VtxFull> > &polys, MeshBase &mesh, MeshFlag flag_and, Flt weld_pos_eps=EPS, Bool convex=false, C Byte *poly_flags=null); // triangulate set of 'polys' to 'mesh', 'mesh' will be deleted at start, 'flag_and'=elements to include in 'mesh' creation, 'weld_pos_eps'=epsilon used for final vertex position welding, 'convex'=if 'polys' are known to be convex (this will speed up the process), 'poly_flags'=if not null then mesh faces will have their flags set according to given pointer (its length must be as long as number of 'polys')
