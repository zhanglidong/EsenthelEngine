...
struct Object // Object Parameters - they're created in the Editor and are used for creating typical Game Objects 'Game.Obj'
{
 ...
 C Param * findParam (CChar8 *name)C; // find parameter by name, first local parameters are searched, if not found then bases are searched recursively, null on fail !! each objects parameters are required to be sorted according to their name !!
   Param * findParam (C Str8 &name) ; // find parameter by name, first local parameters are searched, if not found then bases are searched recursively, null on fail !! each objects parameters are required to be sorted according to their name !!
 C Param * findParam (C Str8 &name)C; // find parameter by name, first local parameters are searched, if not found then bases are searched recursively, null on fail !! each objects parameters are required to be sorted according to their name !!
