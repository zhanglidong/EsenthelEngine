...
struct Cloth // Physical Cloth
{
 ...
 C Particle* lockRead(); // access cloth particles for reading, length of returned array is equal to 'vtxs', after calling this method you need to call 'unlock'
   ...
   void drawPhysical       (                               C Vec   &pos_delta=VecZero, C Vec &ang_delta=VecZero)C; // draw physically simulated cloth part                  , automatically uses Frustum culling, this can be called only in RM_PREPARE
