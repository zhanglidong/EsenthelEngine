...
const uint ProjectVersion     =66, // !! increase this by one if any of engine/editor asset formats have changed !!
           ClientServerVersion=63; // !! client/server version (client will fail if tries to connect to server compiled with different version), increase this by one if any of engine resource formats have changed or if the network protocol has changed or if editor classes formats have changed !!
...
const bool          MiscOnTop=false,
                    ...
                    ForceHQMtrlBase0     =false, // if always use high quality compression for Material Base0  Texture (RGBA/RGB Glow      ) #MaterialTextureChannelOrder
                    ForceHQMtrlBase1     =true , // if always use high quality compression for Material Base1  Texture (NxNy               ) #MaterialTextureChannelOrder, set to true because normals need this (without this, they get very blocky due to low quality)
                    ForceHQMtrlBase2     =false, // if always use high quality compression for Material Base2  Texture (SmoothReflBumpAlpha) #MaterialTextureChannelOrder
                    ForceHQMtrlDetail    =true , // if always use high quality compression for Material Detail Texture (NxNyColBump        ) #MaterialTextureChannelOrder, set to true because normals need this (without this, they get very blocky due to low quality)
