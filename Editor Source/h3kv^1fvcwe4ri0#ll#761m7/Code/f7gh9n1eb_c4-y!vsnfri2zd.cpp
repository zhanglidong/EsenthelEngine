...
class HeightBrushClass : Window
{
   class AlignWaypoint
   {
      ...
      void set(EditWaypoint &wp, EditWaypointPoint &p) {T.wp=&wp; T.p=&p;}
   }
   Image               noise[2];
   Tabs                mode;
   ...
   static void ModeChanged(HeightBrushClass &hb) {SetHmOp((hb.mode()>=0) ? OP_HM_HEIGHT : OP_HM_NONE);}
   ...
   HeightBrushClass& create()
   {
      ...
      clientRect(Rect_RU(WorldEdit.rect().w(), -defaultBarHeight(), 0.22, 0.32));
      T+=mode.create(Rect_LU(0, clientSize()).extend(-0.01), 0, text, Elms(text)).func(ModeChanged, T);
   #if MOBILE
   ...
   static void GetHeight(Cell<Area> &cell, Cursor &cur, int thread_index) {HeightBrush.getHeight(cell(), cur);}
   ...
   static void UpdateHeight(Cell<Area> &cell, ptr user, int thread_index) {HeightBrush.updateHeight(cell());}
          void updateHeight(     Area  &area)
   {
      if(area.hm && Brush.affected(area.xy))
      {
         ...
         Heightmap2 &hm       =*area.hm;
         int         res      =WorldEdit.hmRes();
         flt         noise_mul=Min(WorldEdit.areaSize()/(res-1)*2, 1.0); // use 1.0 limit because at greater scales the noise is too sharp
            ...
            if(flt d=Brush.powerTotal(pos))
            {
               ...
               switch(mode())
               {
                  ...
                  case HB_SOFT   :
                  {
                     ...
                     AdjustValTime(pos_y, sum*0.25, Max(Lerp(1.0, 0.5, d), 0.1), speed);
                  }break;
               }
               hm.height(x, y, pos_y);
      ...
      REPA(WorldEdit.visible_waypoints)
      {
         EditWaypoint &wp=*WorldEdit.visible_waypoints[i]; if(!wp.removed) //if(Cuts(wp.area, rect))
         {
            REPA(wp)
            {
               ...
               {
                  if(Abs(p.pos.y-WorldEdit.hmHeight(p.pos))<=0.1)align_waypoints.New().set(wp, p);
               }
            }
         }
      }
