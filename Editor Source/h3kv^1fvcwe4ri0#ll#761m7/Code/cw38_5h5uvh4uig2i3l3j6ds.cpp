 ...
 C MeshPart* getPart(int i)C {return getLod().parts.addr(i);}
   ...
   void createMesh()
   {
...

      {
         ...
         n.New().create("Brighten Vertex Colors"        , MeshColorBrghtn, T).kbsc(KbSc(KB_C, KBSC_CTRL_CMD|KBSC_SHIFT         )).desc("This option will brighten vertex colors");
         n.New().create("Darken Vertex Colors"          , MeshColorDarken, T).kbsc(KbSc(KB_C, KBSC_CTRL_CMD|KBSC_SHIFT|KBSC_ALT)).desc("This option will darken vertex colors");
...
cur_skel_to_saved_skel.renameBone(old_name, new_name);
   ...
   static void NewBox    (ObjView &editor) {if(editor.getPhysElm() && editor.phys){editor.phys_undos.set("box"    ); if(editor.hasPhysMeshOrConvex())editor.physDel(); MeshBase m; editor.physMesh(m); Box  box =m;                           {CacheLock cl(PhysBodies); editor.phys->parts.New().create(OBox(Box(Max(box.w(), 0.1), Max(box.h(), 0.1), Max(box.d(), 0.1), box.center()))); editor.phys->setBox();} editor.setChangedPhys();}}
   static void NewBall   (ObjView &editor) {if(editor.getPhysElm() && editor.phys){editor.phys_undos.set("ball"   ); if(editor.hasPhysMeshOrConvex())editor.physDel(); MeshBase m; editor.physMesh(m); Ball ball=m;       MAX(ball.r , 0.05); {CacheLock cl(PhysBodies); editor.phys->parts.New().create(ball                                                                            ); editor.phys->setBox();} editor.setChangedPhys();}}
   static void NewCapsule(ObjView &editor) {if(editor.getPhysElm() && editor.phys){editor.phys_undos.set("capsule"); if(editor.hasPhysMeshOrConvex())editor.physDel(); MeshBase m; editor.physMesh(m); Box  box =m; flt h=Max(box.h(), 0.1 ); {CacheLock cl(PhysBodies); editor.phys->parts.New().create(Capsule(h*0.25, h, box.center())                                                ); editor.phys->setBox();} editor.setChangedPhys();}}
   static void NewTube   (ObjView &editor) {if(editor.getPhysElm() && editor.phys){editor.phys_undos.set("tube"   ); if(editor.hasPhysMeshOrConvex())editor.physDel(); MeshBase m; editor.physMesh(m); Box  box =m; flt h=Max(box.h(), 0.1 ); {CacheLock cl(PhysBodies); editor.phys->parts.New().create(Tube   (h*0.25, h, box.center())                                                ); editor.phys->setBox();} editor.setChangedPhys();}}
   ...
   void physSetMesh()
   {
      ...
      {
         phys_undos.set("mesh");
         MeshBase m; physMesh(m); m.weldVtx(0, 0.005).simplify(1, 0.01, 1, 1, 1, 1, PI, true, SIMPLIFY_PLANES);
