...
class Project
{
 ...
 C Elm* findElm  (C UID &id  , ELM_TYPE type)C{if(C Elm *elm=findElm(id  ))if(elm.type==type)return elm   ; return null   ;}
   ...
   static Vec2  LerpToMad(flt from, flt to) {return Vec2(to-from, from);}
   static Vec2 ILerpToMad(flt from, flt to) {return Vec2(1/(to-from), from/(from-to));}
   static flt   FloatSelf(flt x) {return x;}
   static flt   PowMax   (flt x, flt y) {return (x<=0) ? 0 : Pow(x, y);}
   static void IncludeAlpha(Image &image) {image.copyTry(image, -1, -1, -1, ImageTypeIncludeAlpha(image.type()));}
   ...
   static void TransformImage(Image &image, C MemPtr<TextParam> &params, bool clamp)
   {
      ...
      FREPA(params)
      {
         ...
         if(param.name=="scale") // the formula is ok (for normal too), it works as if the bump was scaled vertically by 'scale' factor
         {
            ...
            {
               if(image.monochromatic())image.mulAdd(Vec4(Vec(scale), 1), Vec4(Vec(-0.5*scale+0.5), 0), &box);else // if image is monochromatic then we need to transform all RGB together
         ...
         if(param.name=="scaleXY")
         {
            ...
            // v2=(v2-0.5)*r+0.5
            // v2=v2*r -0.5*r+0.5
            if(image.monochromatic()){flt a=r.avg(); image.mulAdd(Vec4(Vec(a), 1), Vec4(Vec(-0.5*a+0.5)  , 0), &box);} // if image is monochromatic then we need to transform all RGB together
