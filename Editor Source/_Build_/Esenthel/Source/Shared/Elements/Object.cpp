...
void Adjust(Particles &particles, C EditObject &obj, C Matrix &matrix, Project &proj) // adjust particles without resetting them because this can be called each frame when editing objects
{
   if(obj.editor_type==EditObjType.elmID(EDIT_OBJ_PARTICLES))
   {
      ...
      if(C EditParam *p=obj.findParam("Radius"            ))particles.radius          =    p->asFlt  ()           ;else particles.radius=0.048f;
      ...
      if(C EditParam *p=obj.findParam("Life"              ))particles.life            =Max(p->asFlt  (), 0       );else particles.life=1.31f;
      if(C EditParam *p=obj.findParam("Life Random"       ))particles.life_random     =Max(p->asFlt  (), 0       );else particles.life_random=1.31f;
      ...
      switch(shape_type)
      {
         ...
         case SHAPE_TORUS  : particles.source(Torus(0.3f, 0.1f)); break;
...
/******************************************************************************/
   EditObjTypeClass::EditObjTypeClass()
   {
      Enum::Elm elms[]=
      {
      ...
      }
      {
         ...
         {GuiEditParam &p=ps.New(); p.name="Falloff"     ; p.type=PARAM_FLT  ; p.value.f=0.5f  ; p.id.set(4, 0, 0, 0); p.priority=pr++;}
      }
      {
         ...
         {GuiEditParam &p=ps.New(); p.name="Image Speed"       ; p.type=PARAM_FLT  ; p.value.f =1.0f             ; p.id.set( 8, 0, 0, 0); p.priority=pr++; p.min(0).desc("Image animation speed, 0..Inf, default=1");}
         ...
         {GuiEditParam &p=ps.New(); p.name="Radius"            ; p.type=PARAM_FLT  ; p.value.f =0.048f           ; p.id.set(15, 0, 0, 0); p.priority=pr++; p.min(0).mouseEditMode(PROP_MOUSE_EDIT_SCALAR).desc("Particle radius at its creation time, 0..Inf\nThis is the initial value of 'Particle.radius' at its creation time.");}
         ...
         {GuiEditParam &p=ps.New(); p.name="Radius Growth"     ; p.type=PARAM_FLT  ; p.value.f =1.0f             ; p.id.set(16, 0, 0, 0); p.priority=pr++; p.min(0).mouseEditMode(PROP_MOUSE_EDIT_SCALAR).desc("Particle radius growth factor, 0..Inf, default=1\nThis affects 'Particle.radius' in each frame, where a value of:\n0..1 - decreases the radius\n1 - keeps it constant\n1..Inf - increases the radius");}
         {GuiEditParam &p=ps.New(); p.name="Offset Range"      ; p.type=PARAM_FLT  ; p.value.f =0.0f             ; p.id.set(17, 0, 0, 0); p.priority=pr++; p.min(0).desc("Particle position offset range applied during drawing, 0..Inf, default=0\nThis simulates the range of random position offsets for each particle during drawing\nvalue of 0 is the fastest");}
         {GuiEditParam &p=ps.New(); p.name="Offset Speed"      ; p.type=PARAM_FLT  ; p.value.f =1.0f             ; p.id.set(18, 0, 0, 0); p.priority=pr++; p.min(0).desc("Particle position offset speed applied during drawing, 0..Inf, default=1\nThis simulates the speed of random potision offsets for each particle during drawing");}
         {GuiEditParam &p=ps.New(); p.name="Life"              ; p.type=PARAM_FLT  ; p.value.f =1.31f            ; p.id.set(19, 0, 0, 0); p.priority=pr++; p.min(0).desc("Particle life, 0..Inf\nThis is the life of a single particle 'Particle.max_life'");}
         {GuiEditParam &p=ps.New(); p.name="Life Random"       ; p.type=PARAM_FLT  ; p.value.f =0               ; p.id.set(33, 0, 0, 0); p.priority=pr++; p.min(0).desc("Particle life random factor, 0..Inf\nThis affects 'Particle.max_life'\n0 value keeps it constant\n1 value can make it 2x shorter to 2x longer\n2 value can make it 3x shorter to 3x longer");}
         {GuiEditParam &p=ps.New(); p.name="Glue"              ; p.type=PARAM_FLT  ; p.value.f =0.0f             ; p.id.set(20, 0, 0, 0); p.priority=pr++; p.range(0, 1).desc("Moves particles along with matrix movement, 0..1, default=0\nThis affects each single particle position according to source generation object movement, value of 0 disables position adjustment and doesn't require any additional calculations");}
         {GuiEditParam &p=ps.New(); p.name="Velocity Damping"  ; p.type=PARAM_FLT  ; p.value.f =0.0f             ; p.id.set(21, 0, 0, 0); p.priority=pr++; p.range(0, 1).desc("Particle velocity damping, 0..1, default=0\nThis dampens particles velocity in each frame");}
         {GuiEditParam &p=ps.New(); p.name="Angular Velocity"  ; p.type=PARAM_FLT  ; p.value.f =0.0f             ; p.id.set(22, 0, 0, 0); p.priority=pr++; p.min  (0   ).desc("Determines the 'Particle.ang_vel' to simulate particle rotations, 0..Inf, default=0");}
         {GuiEditParam &p=ps.New(); p.name="Random Velocity"   ; p.type=PARAM_FLT  ; p.value.f =0.084f           ; p.id.set(23, 0, 0, 0); p.priority=pr++; p.min  (0   ).desc("Length of initial velocity applied at a random direction at particle creation, 0..Inf, default=0");}
         {GuiEditParam &p=ps.New(); p.name="Initial Velocity X"; p.type=PARAM_FLT  ; p.value.f =0.0f             ; p.id.set(24, 0, 0, 0); p.priority=pr++; p.desc("Initial velocity applied at particle creation");}
         {GuiEditParam &p=ps.New(); p.name="Initial Velocity Y"; p.type=PARAM_FLT  ; p.value.f =0.0f             ; p.id.set(25, 0, 0, 0); p.priority=pr++; p.desc("Initial velocity applied at particle creation");}
         {GuiEditParam &p=ps.New(); p.name="Initial Velocity Z"; p.type=PARAM_FLT  ; p.value.f =0.0f             ; p.id.set(26, 0, 0, 0); p.priority=pr++; p.desc("Initial velocity applied at particle creation");}
         {GuiEditParam &p=ps.New(); p.name="Acceleration X"    ; p.type=PARAM_FLT  ; p.value.f =0.0f             ; p.id.set(27, 0, 0, 0); p.priority=pr++; p.desc("Acceleration applied each frame");}
         {GuiEditParam &p=ps.New(); p.name="Acceleration Y"    ; p.type=PARAM_FLT  ; p.value.f =0.0f             ; p.id.set(28, 0, 0, 0); p.priority=pr++; p.desc("Acceleration applied each frame");}
         {GuiEditParam &p=ps.New(); p.name="Acceleration Z"    ; p.type=PARAM_FLT  ; p.value.f =0.0f             ; p.id.set(29, 0, 0, 0); p.priority=pr++; p.desc("Acceleration applied each frame");}
 ...
 C Memc<GuiEditParam>* EditObjTypeClass::findParams(C UID &type_id)
   {
      int type=find(type_id);
      return InRange(type, params) ? &params[type] : null;
   }
   TerrainObj::operator bool()C {return obj!=null;}
   bool TerrainObj::operator==(C TerrainObj &to)C {return obj==to.obj;}
   bool TerrainObj::operator!=(C TerrainObj &to)C {return obj!=to.obj;}
   void TerrainObj::set(OBJ_ACCESS access, C EditObjectPtr &obj) {if(access==OBJ_ACCESS_TERRAIN && obj)T.obj=obj;else T.obj=null;}
   void TerrainObj::operator=(C ObjData &obj                         ) {if(obj.removed)T.obj=null;else set(obj.params.access, obj.params.base);}
   TerrainObj::TerrainObj(OBJ_ACCESS access, C EditObjectPtr &obj) {set(access, obj);}
   TerrainObj::TerrainObj(C ObjData &obj                         ) {T=obj;}
   TerrainObj2::operator bool()C {return mesh || phys;}
   bool TerrainObj2::operator==(C TerrainObj2 &to)C {return mesh==to.mesh && phys==to.phys;}
   bool TerrainObj2::operator!=(C TerrainObj2 &to)C {return mesh!=to.mesh || phys!=to.phys;}
   void TerrainObj2::operator=(C Object &obj) {if(obj.access()==OBJ_ACCESS_TERRAIN){T.mesh=obj.mesh(); T.phys=obj.phys();}else{T.mesh=null; T.phys=null;}}
   TerrainObj2::TerrainObj2() {}
   TerrainObj2::TerrainObj2(C Object &obj) {T=obj;}
   PhysPath::operator bool()C {return mode!=OBJ_PATH_IGNORE;}
   bool PhysPath::operator==(C PhysPath &pp)C {return mode==pp.mode && body==pp.body;}
   bool PhysPath::operator!=(C PhysPath &pp)C {return mode!=pp.mode || body!=pp.body;}
   void PhysPath::set(OBJ_PATH mode, C PhysBodyPtr &body) {if(mode!=OBJ_PATH_IGNORE && body){T.mode=mode; T.body=body;}else{T.mode=OBJ_PATH_IGNORE; T.body=null;}}
   void PhysPath::operator=(C Object  &obj                    ) {set(obj.path(), obj.phys());}
   void PhysPath::operator=(C ObjData &obj                    )
   {
   ...
   PhysPath::PhysPath(OBJ_PATH mode, C PhysBodyPtr &body) : mode(OBJ_PATH_IGNORE) {set(mode, body);}
   PhysPath::PhysPath(C Object  &obj                    ) : mode(OBJ_PATH_IGNORE) {T=obj;}
   PhysPath::PhysPath(C ObjData &obj                    ) : mode(OBJ_PATH_IGNORE) {T=obj;}
   bool ObjData::old(C TimeStamp &now)C {return matrix_time<now && removed_time<now && params.old();}
   bool ObjData::equal(C ObjData &src)C {return matrix_time==src.matrix_time && removed_time==src.removed_time && params.equal(src.params);}
   bool ObjData::newer(C ObjData &src)C {return matrix_time> src.matrix_time || removed_time> src.removed_time || params.newer(src.params);}
   TerrainObj ObjData::terrainObj()C {return T;}
   PhysPath   ObjData::physPath()C {return T;}
   uint ObjData::memUsage()C {return params.memUsage();}
   void ObjData::setRemoved(bool removed, C TimeStamp &time) {T.removed=removed; T.removed_time=time;}
   void ObjData::create(C ObjData &src)
   {
      params.create(src.params);
      removed=src.removed; removed_time=src.removed_time;
      matrix =src.matrix ;  matrix_time=src. matrix_time;
   }
   bool ObjData::sync(C ObjData &src, cchar *edit_path)
   {
   ...
   bool ObjData::undo(C ObjData &src, cchar *edit_path)
   {
   ...
   void ObjData::create(C Game::Area::Data::AreaObj &src, C UID &type, C EditObjectPtr &base, C TimeStamp &time)
   {
      removed=false;
      id=src.id;
   ...
   void ObjData::copyTo(Game::Area::Data::AreaObj &dest, C Project &proj)C
   {
   ...
   ObjData& ObjData::create(C Edit::WorldObjParams &src, C Str &edit_path)
   {
   ...
   bool ObjData::save(File &f, cchar *edit_path)C
   {
      f.cmpUIntV(0);
   ...
   bool ObjData::load(File &f, cchar *edit_path)
   {
      switch(f.decUIntV())
      {
         case 0:
...
PhysPath::PhysPath() : mode(OBJ_PATH_IGNORE) {}

ObjData::ObjData() : removed(false), id(UID().randomizeValid()), matrix(1) {}
