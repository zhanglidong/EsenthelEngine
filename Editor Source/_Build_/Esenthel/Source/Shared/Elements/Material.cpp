...
/******************************************************************************/
   ...
   bool EditMaterial::hasBase1Tex()C {return hasNormalMap() || specular_map.is() || glow_map.is();}
   uint EditMaterial::baseTex()C {return (color_map.is() ? BT_COLOR : 0)|(alpha_map.is() ? BT_ALPHA : 0)|(hasBumpMap() ? BT_BUMP : 0)|(hasNormalMap() ? BT_NORMAL : 0)|(specular_map.is() ? BT_SPECULAR : 0)|(glow_map.is() ? BT_GLOW : 0);}
   ...
   bool EditMaterial::usesTexNormal()C {return  rough    >EPS_COL                 && hasNormalMap();}
   ...
   void EditMaterial::resetAlpha()
   {
      switch(tech)
      {
         ...
         case MTECH_GRASS     :
         case MTECH_LEAF      : color_s.w=0.5f; break;
   ...
   void EditMaterial::separateBaseTexs(C Project &proj, C TimeStamp &time)
   {
...
EditMaterial::EditMaterial() : flip_normal_y(false), cull(true), tex_quality(0), downsize_tex_mobile(0), tech(MTECH_DEFAULT), color_s(1, 1, 1, 1), ambient(0, 0, 0), specular(0), sss(0), glow(0), rough(0), bump(0), tex_scale(1), det_scale(4), det_power(0.3f), reflection(0.2f), base_0_tex(UIDZero), base_1_tex(UIDZero), detail_tex(UIDZero), macro_tex(UIDZero), reflection_tex(UIDZero), light_tex(UIDZero) {}
