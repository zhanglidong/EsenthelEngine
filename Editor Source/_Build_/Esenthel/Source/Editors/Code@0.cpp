   ...
   void CodeView::draw(C GuiPC &gpc)
{
   ...
   uint AppPropsEditor::OrientToFlag(ORIENT orient)
   {
      ...
      void AppPropsEditor::AppImage::setImage()
      {
         ...
         if(image && image->is() && image->mode()!=IMAGE_2D)
         {
            ...
            image->copyTry(image_2d, Max(size.x, 1), Max(size.y, 1), 1, ImageTypeUncompressed(image->type()), IMAGE_2D, 1, FILTER_BEST, true, true);
            image=&image_2d;
         }
   ...
   p_notification_icon=&add("Notification Icon");
      ...
      pos.y-=h;
      am_ai_g.pos(pos).parent(platforms.tab(3)); platforms.tab(3)+=am_ai_g.button.create(Rect_RU(am_ai_g.textline.rect().ru()+Vec2(th, 0), th*2, th), "Get").func(GetAdMobApp); am_ai_g.textline.rect(Rect(am_ai_g.textline.rect().ld(), am_ai_g.button.rect().lu()));
      am_ai_i.pos(pos).parent(platforms.tab(4)); platforms.tab(4)+=am_ai_i.button.create(Rect_RU(am_ai_i.textline.rect().ru()+Vec2(th, 0), th*2, th), "Get").func(GetAdMobApp); am_ai_i.textline.rect(Rect(am_ai_i.textline.rect().ld(), am_ai_i.button.rect().lu()));
      pos.y-=h;
