...
draw_at_distance_prop=&props.New().create("Draw at Distance", MEMBER(NewLodClass, draw_at_distance)).desc("Draw LOD at specified distance\nKeyboard Shortcut: Alt+D");
   draw_distance_prop=&props.New().create("Draw Distance"   , MEMBER(NewLodClass, draw_distance   )).min(0).mouseEditMode(PROP_MOUSE_EDIT_SCALAR).mouseEditSpeed(0.5f);
                     ...
                     //props.New().create("Max Material"    , MEMBER(NewLodClass, max_material    )).range(0,  1).mouseEditSpeed(0.1).desc("Max allowed vertex material deviations (0..1)");  // this is not used since most likely there won't be any models with per-vertex materials over here
                       ...
                       props.New().create("Max Normal"      , MEMBER(NewLodClass, max_normal      )).range(0, 90).mouseEditSpeed(5.5f).desc("Max allowed vertex normal angle deviations (0..90)");
...
NewLodClass::NewLodClass() : preview(true), simplified_valid(false), processed_ready(false), keep_border(false), draw_at_distance(false), finished(false), stop(false), intensity(0.5f), max_distance(1), max_uv(1), max_color(1), max_material(1), max_skin(1), max_normal(90), draw_distance(2), scale(1), src_id(0), simplified_src_id(0), change_id(0), simplified_change_id(0), quality(null), preview_prop(null), draw_at_distance_prop(null) {}
