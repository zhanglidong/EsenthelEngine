/******************************************************************************/
   template<int i>   void EnvEditor::CloudsColor(  EditEnv &env, C Str &text) {env.clouds.layers[i].color_s=TextVec4(text); env.clouds_color_time[i].getUTC();}
   template<int i>   Str  EnvEditor::CloudsColor(C EditEnv &env             ) {return env.clouds.layers[i].color_s;}
