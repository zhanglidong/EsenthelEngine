...
/******************************************************************************/
      ::SelectionClass::SEL_MODE SelectionClass::TouchSelTabs::selMode(           ) {return InRange(sel, SM_NUM) ?  SEL_MODE(sel) : SM_SET;}
      int      SelectionClass::TouchSelTabs::operator()(           ) {return sel;}
    C Rect&    SelectionClass::TouchSelTabs::tabRect(int i      ) {return rects[i];}
      int      SelectionClass::TouchSelTabs::tabIndex(C Vec2 &pos)
      {
         if(rect().w()>=rect().h()){FREPA(rects)if(pos.x<=rects[i].max.x)return i;}
         else                      {FREPA(rects)if(pos.y>=rects[i].min.y)return i;}
         return rects.elms()-1;
      }
      void SelectionClass::TouchSelTabs::image(int i, C ImagePtr &image) {if(InRange(i, images))images[i]=image;}
      void SelectionClass::TouchSelTabs::clear() {keep=-1; keep_on_touch_start=sel;}
      ::SelectionClass::TouchSelTabs& SelectionClass::TouchSelTabs::create(int elms)
      {
         images.setNum(elms);
         rects .setNum(elms);
         super::create();
      ...
      ::SelectionClass::TouchSelTabs& SelectionClass::TouchSelTabs::move(C Vec2 &delta)
{
      ...
      void SelectionClass::TouchSelTabs::update(C GuiPC &gpc)
{
      ...
      void SelectionClass::TouchSelTabs::draw(C GuiPC &gpc)
{
   ...
   int  SelectionClass::elms(     )C {return  objs.elms();}
   Obj& SelectionClass::operator[](int i)  {return *objs[i];}
   bool SelectionClass::has(C UID &world_obj_instance_id)C {if(world_obj_instance_id.valid())REPA(objs)if(objs[i]->id==world_obj_instance_id)return true; return false;}
   bool SelectionClass::selRect(Rect &rect)
   {
      if(selecting)
      {
   ...
   void SelectionClass::resize()
   {
      ...
      }else
      {
         ...
         {
            tsm.rect(Rect_C(0, -D.h()*0.6f, l, h)); // slightly above bottom
         }
      }
   }
   ...
   void SelectionClass::removed(Obj &obj) {deselect(obj); unhighlight(obj);}
   void SelectionClass::setUndo() {REPAO(T).setUndo();}
   void SelectionClass::update()
   {
   ...
   void SelectionClass::draw(Edit::Viewport4::View &view)
   {
      // draw selection rectangle
      Rect rect; if(sel_view==&view && selRect(rect))
      {
         rect.draw(ColorAlpha(ColorBrightness(LitColor, 0.25f), 0.1f), true );
         rect.draw(ColorAlpha(ColorBrightness(LitColor, 0.80f), 0.6f), false);
      }
   }
   void SelectionClass::drawSelLit()
   {
      REPAO(objs)->drawSelected();
      REPA (highlighted)if(Obj *obj=highlighted[i])if(!obj->selected)obj->drawSelected();
   }
  SelectionClass::~SelectionClass() {REPA(objs)deselect(*objs[i]);}
SelectionClass::SelectionClass() : selecting(false), disable_selection(false), tsm_side(false), sel_changed(false), sel_view(null) {}

SelectionClass::TouchSelTabs::TouchSelTabs() : sel(-1), keep(-1), keep_on_touch_start(-1) {}
