         ...
         switch(water_op())
         {
            ...
            case WAO_RIVER_SIZE      :
            {
               ...
               if(Ms.b(1) && (lake || river))
               {
                  ...
                  if(!v4.perspective() || !v4.fpp())water_edit_speed=CamMoveScale(v4.perspective());else if(first_push)water_edit_speed=Dist(lake_pos ? *lake_pos : *river_pos, ActiveCam.matrix.pos)*0.75f; // in FPP mode automatically detect the speed according to waypoint distance to camera, but only at the moment of first button push
