...
void UpdateGameCam()
{
   ...
   Cam.transformByMouse(0.1f, 1000, CAMH_ZOOM | (((Ms.b(0) || Ms.b(MOUSE_MAXIMIZE)) && (!Gui.ms() || Gui.ms()==Gui.desktop())) ? CAMH_MOVE_XZ : (Ms.b(1) ? CAMH_MOVE : CAMH_ROT)));
}
bool UpdateGame()
{
...
void DrawGame()
{
   ...
   if(DrawPhysics || DrawWorldPath)
   {
      ...
      if(DrawWorldPath)Game::World.path().draw(64, 0.5f);
   }
